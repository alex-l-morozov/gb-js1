...
function init() {
    ...
    */
    ...
    document.getElementById('snake-score').innerText = "Ваш результат: " + score.toString();
...
function move() {
    ...
    //console.log(new_unit);
    if (!isSnakeUnit(new_unit) && !haveWall(new_unit)) {
...
function createWall() {
    ...
    while (!wallCreated) { //пока стену не создали
        ...
        let wall_cell = document.getElementsByClassName('cell-' + wall_y + '-' + wall_x)[0];
        let wall_cell_classes = wall_cell.getAttribute('class').split(' ');

        // проверка на змейку
        if (!wall_cell_classes.includes('snake-unit')) {
            let classes = '';
            for (let i = 0; i < wall_cell_classes.length; i++) {
